"""
MOBILENET-UNET WITH MSCA + LBFR + PPM (TRIPLE ATTENTION MODULES)
=================================================================
...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_name, 
    ...
    print(f"  Architecture: Deeper Decoder + MC Dropout + MSCA + LBFR + PPM")
    ...
    print(f"  PPM: Pyramid Pooling Module (scales [1×1, 2×2, 3×3, 6×6])")
    print(f"  Enhancement: Sequential MSCA → LBFR → PPM per decoder block")
    print(f"  Total Attention Blocks: 12 (4 MSCA + 4 LBFR + 4 PPM)")
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*100}")
    print(f"{'MOBILENET-UNET WITH MSCA + LBFR + PPM (TRIPLE ATTENTION MODULES)':^100s}")
    print(f"{'='*100}")
    ...
    model, history, params = train_model(
        ...
        "Deeper_Focal_MC_MSCA_LBFR_PPM",
        augment=train_aug, save_dir=save_dir
    )
    
    # Evaluate
    results = evaluate_model(model, X_test, y_test, "Deeper+Focal+MC+MSCA+LBFR+PPM", run_mc_dropout=True)
    
    print(f"\n{'='*100}")
    print(f"{'✓ TRAINING AND EVALUATION COMPLETED!':^100s}")
    print(f"{'='*100}")
    print(f"\nModel: Deeper + Focal + EIoU + MC Dropout + MSCA + LBFR + PPM")
    print(f"Triple Enhancement:")
    print(f"  1. MSCA: Multi-scale dilated convolutions with rates [1, 2, 3, 5]")
    print(f"  2. LBFR: Channel + Spatial attention + Recalibration")
    print(f"  3. PPM: Pyramid pooling with scales [1×1, 2×2, 3×3, 6×6]")
    print(f"Enhancement Strategy: Sequential MSCA → LBFR → PPM per decoder block")
    print(f"Context Coverage: Local Multi-scale + Attention + Global Multi-scale")
    print(f"Total Attention Blocks: 12 (4 MSCA + 4 LBFR + 4 PPM)")
