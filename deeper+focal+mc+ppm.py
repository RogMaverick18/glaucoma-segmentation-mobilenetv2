"""
MOBILENET-UNET WITH PPM (PYRAMID POOLING MODULE)
=================================================
Enhanced with PPM from PSPNet for multi-scale global context aggregation

PPM Module:
- Multi-scale adaptive pooling (scales: 1×1, 2×2, 3×3, 6×6)
- Captures global context at different pyramid levels
- Feature fusion with concatenation
- From PSPNet architecture, used in ODFormer

Configuration:
- Base: Deeper Decoder + Focal-EIoU + MC Dropout
- Enhancement: PPM on decoder outputs
- PPM Placement: After each decoder block (4 total)
"""
...
# ============================================================================
# MODEL ARCHITECTURE WITH PPM
# ============================================================================

def enhanced_decoder_block_with_ppm(x, skip_connection, filters, block_name, dropout_rate=0.3):
    """Enhanced decoder block with PPM for multi-scale global context"""
    ...
    # Apply PPM for multi-scale global context aggregation
    x = ppm_block(x, filters, pool_scales=[1, 2, 3, 6], block_name=f'{block_name}_ppm')
    
    return x

def build_unet_with_ppm(input_shape=(256, 256, 3), num_classes=3, dropout_rate=0.3):
    """Build MobileNetV2-UNet with PPM modules"""
    ...
    # Decoder with PPM modules
    x = enhanced_decoder_block_with_ppm(bridge, skip_4, 512, 'decoder_1', dropout_rate)
    x = enhanced_decoder_block_with_ppm(x, skip_3, 256, 'decoder_2', dropout_rate)
    x = enhanced_decoder_block_with_ppm(x, skip_2, 128, 'decoder_3', dropout_rate * 0.7)
    x = enhanced_decoder_block_with_ppm(x, skip_1, 64, 'decoder_4', dropout_rate * 0.5)
    ...
    print(f"  PPM Modules: 4 (one per decoder block)")
    print(f"  PPM Pyramid Scales: [1×1, 2×2, 3×3, 6×6]")
    print(f"  Enhancement: Pyramid Pooling for Multi-scale Global Context")
...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_name, 
    ...
    """Train model with PPM enhancement"""
    print(f"\n{'='*80}")
    print(f"TRAINING: {model_name}")
    print(f"{'='*80}")
    print(f"  Loss: Focal + EIoU")
    print(f"  Architecture: Deeper Decoder + MC Dropout + PPM")
    print(f"  PPM: Pyramid Pooling Module from PSPNet")
    
    K.clear_session()
    
    # Build model
    model = build_unet_with_ppm(dropout_rate=0.3)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*100}")
    print(f"{'MOBILENET-UNET WITH PPM (PYRAMID POOLING MODULE)':^100s}")
    print(f"{'='*100}")
    ...
    # Evaluate
    results = evaluate_model(model, X_test, y_test, "Deeper+Focal+MC+PPM", run_mc_dropout=True)
    
    print(f"\n{'='*100}")
    print(f"{'✓ TRAINING AND EVALUATION COMPLETED!':^100s}")
    print(f"{'='*100}")
    print(f"\nModel: Deeper + Focal + EIoU + MC Dropout + PPM")
    print(f"Enhancement: Pyramid Pooling Module from PSPNet")
    print(f"PPM Pyramid Scales: [1×1, 2×2, 3×3, 6×6]")
