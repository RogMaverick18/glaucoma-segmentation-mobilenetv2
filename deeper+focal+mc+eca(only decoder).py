"""
COMBINED ADVANCED MOBILENET-UNET WITH DECODER-ONLY ECA ATTENTION
==================================================================
This combines:
1. Deeper decoder architecture with enhanced blocks
2. Focal-EIoU loss
3. MC Dropout for uncertainty estimation
4. ECA (Efficient Channel Attention) - DECODER-ONLY VERSION
   - Applied ONLY to: Decoder blocks (4 locations)
   - REMOVED from: Skip connections, bridge, final layer
   - This applies attention to decoder features after skip concatenation

STRATEGY: Place ECA only in decoder blocks to refine decoder features
after they merge with encoder skip connections.
"""
...
# ============================================================================
# ENHANCED DECODER BLOCK WITH ECA
# ============================================================================

def enhanced_decoder_block_with_eca(x, skip_connection, filters, block_name, dropout_rate=0.3):
    """Enhanced decoder block WITH ECA attention"""
    ...
    # Apply ECA attention to decoder features
    x = eca_block(x, name=block_name)
    
    return x

# ============================================================================
# MODEL: DECODER-ONLY ECA VERSION
# ============================================================================

def build_combined_unet_decoder_only_eca(input_shape=(256, 256, 3), num_classes=3, 
                                         use_deeper=False, dropout_rate=0.3):
    """
    Combined MobileNet-UNet with DECODER-ONLY ECA
    
    ECA Placement (DECODER-ONLY VERSION):
    - Skip connections: NO ECA ❌
    - Bridge: NO ECA ❌
    - Decoder_1 (16x16): YES ✅
    - Decoder_2 (32x32): YES ✅
    - Decoder_3 (64x64): YES ✅
    - Decoder_4 (128x128): YES ✅
    - Final layer: NO ECA ❌
    Total: 4 ECA blocks (on decoder blocks only)
    
    Rationale: Apply attention to decoder features after they merge with encoder features,
    allowing the network to refine the fused representation.
    """
    
    print(f"\nBuilding Combined UNet with DECODER-ONLY ECA...")
    print(f"  Deeper Architecture: {use_deeper}")
    print(f"  MC Dropout Rate: {dropout_rate}")
    print(f"  ECA Attention: DECODER-ONLY (4 blocks on decoder blocks)")
    
    inputs = layers.Input(input_shape, dtype='float32')
    backbone = MobileNetV2(input_tensor=inputs, weights='imagenet', include_top=False)
    
    # Get skip connections (NO ECA on skip connections)
    ...
    print("  Applying ECA to decoder blocks...")
    print("    decoder_1 (16x16): WITH ECA ✓")
    print("    decoder_2 (32x32): WITH ECA ✓")
    print("    decoder_3 (64x64): WITH ECA ✓")
    print("    decoder_4 (128x128): WITH ECA ✓")
    ...
    # Decoder blocks with ECA
    if use_deeper:
        x = enhanced_decoder_block_with_eca(bridge, skip_4, 512, 'decoder_1', dropout_rate)
        x = enhanced_decoder_block_with_eca(x, skip_3, 256, 'decoder_2', dropout_rate)
        x = enhanced_decoder_block_with_eca(x, skip_2, 128, 'decoder_3', dropout_rate * 0.7)
        x = enhanced_decoder_block_with_eca(x, skip_1, 64, 'decoder_4', dropout_rate * 0.5)
    else:
        # Baseline decoder with ECA
    ...
    print(f"\n  ECA Configuration (DECODER-ONLY):")
    print(f"    Total ECA Blocks: {eca_count}")
    print(f"    ECA Locations: Decoder blocks ONLY (4 blocks)")
    print(f"    Removed ECA from: Skip connections, Bridge, Final layer")
...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_name, 
    ...
    # Build model
    model = build_combined_unet_decoder_only_eca(use_deeper=use_deeper, dropout_rate=dropout_rate)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*100}")
    print(f"{'MOBILENET-UNET WITH DECODER-ONLY ECA ATTENTION':^100s}")
    print(f"{'='*100}")
    ...
    model, history, params = train_model(
        ...
        "Deeper_Focal_EIoU_MC_ECA_DecoderOnly",
        use_focal_eiou=True, use_deeper=True, dropout_rate=0.3,
        augment=train_aug, save_dir=save_dir
    )
    
    # Evaluate
    results = evaluate_model(model, X_test, y_test, "Deeper+Focal+EIoU+MC+ECA(Decoder)", 
                            run_mc_dropout=True, history=history, save_dir=save_dir)
    
    print(f"\n{'='*100}")
    print(f"{'✓ TRAINING AND EVALUATION COMPLETED!':^100s}")
    print(f"{'='*100}")
    print(f"\nModel: Deeper + Focal-EIoU + MC Dropout + ECA Attention (DECODER-ONLY)")
    print(f"Attention: ECA on decoder blocks (4 blocks)")
