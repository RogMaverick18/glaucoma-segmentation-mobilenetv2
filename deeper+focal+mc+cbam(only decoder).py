"""
COMBINED ADVANCED MOBILENET-UNET WITH MINIMAL CBAM ATTENTION
==============================================================
This combines:
1. Deeper decoder architecture with enhanced blocks
2. Focal-EIoU loss
3. MC Dropout for uncertainty estimation
4. CBAM (Convolutional Block Attention Module) - MINIMAL VERSION
   - Applied ONLY to: decoder blocks (4 locations)
   - REMOVED from: skip connections, bridge, final layer
   - This reduces over-regularization while keeping attention where it matters

STRATEGY: Place CBAM only in decoder blocks where features are being refined
...
# ============================================================================
# ENHANCED DECODER BLOCK WITH MINIMAL CBAM
# ============================================================================
...
def enhanced_decoder_block_with_cbam(x, skip_connection, filters, block_name, dropout_rate=0.3):
    ...
    MINIMAL CBAM VERSION:
    - CBAM applied ONLY at the end of each decoder block
    - NO CBAM on skip connections (they come in raw)
    """
    ...
    # CBAM applied ONLY here (at the end of decoder block)
    x = cbam_block(x, ratio=8, name=block_name)
    
    return x

# ============================================================================
# MODEL: MINIMAL CBAM VERSION (4 blocks only in decoder)
# ============================================================================

def build_combined_unet_minimal_cbam(input_shape=(256, 256, 3), num_classes=3, 
                                     use_deeper=False, dropout_rate=0.3):
    """
    Combined MobileNet-UNet with MINIMAL CBAM
    
    CBAM Placement (MINIMAL VERSION):
    - Skip connections: NO CBAM ❌
    - Bridge: NO CBAM ❌
    - Decoder blocks: YES (4 locations) ✅
    - Final layer: NO CBAM ❌
    Total: 4 CBAM blocks (vs 10 in full version)
    
    This reduces over-regularization while keeping attention in decoder
    where features are being refined for segmentation.
    """
    
    print(f"\nBuilding Combined UNet with MINIMAL CBAM...")
    print(f"  Deeper Architecture: {use_deeper}")
    print(f"  MC Dropout Rate: {dropout_rate}")
    print(f"  CBAM Attention: MINIMAL (4 blocks in decoder only)")
    
    inputs = layers.Input(input_shape, dtype='float32')
    backbone = MobileNetV2(input_tensor=inputs, weights='imagenet', include_top=False)
    
    # Get skip connections (NO CBAM on skips)
    ...
    # NO CBAM on bridge in minimal version
    
    if use_deeper:
        print("  Building deeper decoder with MINIMAL CBAM (decoder blocks only)...")
        # CBAM applied inside each enhanced_decoder_block_with_cbam
        x = enhanced_decoder_block_with_cbam(bridge, skip_4, 512, 'decoder_1', dropout_rate)
        x = enhanced_decoder_block_with_cbam(x, skip_3, 256, 'decoder_2', dropout_rate)
        ...
        print("  Building baseline decoder with MINIMAL CBAM...")
        # Baseline decoder with CBAM only at end of each block
    ...
    # Final upsampling - NO CBAM on final layer in minimal version
    x = layers.UpSampling2D((2, 2))(x)
    x = layers.Conv2D(32, (3, 3), activation='relu', padding='same')(x)
    x = layers.BatchNormalization()(x)
    
    # NO CBAM on final layer
    ...
    print(f"\n  CBAM Configuration (MINIMAL):")
    print(f"    Total CBAM Components: {cbam_count}")
    print(f"    CBAM Locations: Decoder blocks only (4)")
    print(f"    Removed CBAM from: Skip connections, Bridge, Final layer")
...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_name, 
    ...
    print(f"  CBAM Attention: ✓ MINIMAL (4 decoder blocks)")
    
    K.clear_session()
    
    model = build_combined_unet_minimal_cbam(use_deeper=use_deeper, dropout_rate=dropout_rate)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*100}")
    print(f"{'EXPERIMENT: DEEPER + FOCAL-EIOU + MC DROPOUT + MINIMAL CBAM':^100s}")
    print(f"{'='*100}")
    ...
    print(f"\n{'='*80}")
    print("EXPERIMENT: FULL COMBINATION + MINIMAL CBAM (4 blocks)")
    print(f"{'='*80}")
    
    model, history, params = train_model(
        X_train, y_train, X_val, y_val, X_test, y_test,
        "FULL_with_MINIMAL_CBAM",
        use_focal_eiou=True, use_deeper=True, dropout_rate=0.3,
        augment=train_aug, save_dir=save_dir
    )
    
    all_results['FULL + MINIMAL CBAM'] = evaluate_model(model, X_test, y_test, "Full with Minimal CBAM", run_mc_dropout=True, save_dir=save_dir)
    all_params['FULL + MINIMAL CBAM'] = params
