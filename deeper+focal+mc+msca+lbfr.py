"""
MOBILENET-UNET WITH MSCA + LBFR (DUAL ATTENTION MODULES)
=========================================================
Enhanced with both MSCA and LBFR from ODFormer for comprehensive feature enhancement
...
- Sequential application: MSCA → LBFR on each decoder block
- From ODFormer architecture combining both attention mechanisms

Configuration:
- Base: Deeper Decoder + Focal-EIoU + MC Dropout
- Enhancement: MSCA + LBFR on decoder outputs
- Placement: Both modules after each decoder block (4 total)
"""
...
# ============================================================================
# MODEL ARCHITECTURE WITH MSCA + LBFR
# ============================================================================

def enhanced_decoder_block_with_msca_lbfr(x, skip_connection, filters, block_name, dropout_rate=0.3):
    """
    Enhanced decoder block with both MSCA and LBFR modules
    
    Sequential enhancement strategy:
    1. Standard decoder convolutions with residual connection
    2. MSCA for multi-scale context aggregation
    3. LBFR for bidirectional feature recalibration
    
    This combines both attention mechanisms for comprehensive feature enhancement
    """
    ...
    # Apply MSCA for multi-scale context aggregation
...
def build_unet_with_msca_lbfr(input_shape=(256, 256, 3), num_classes=3, dropout_rate=0.3):
    """Build MobileNetV2-UNet with both MSCA and LBFR modules"""
    ...
    # Decoder with MSCA + LBFR modules
    x = enhanced_decoder_block_with_msca_lbfr(bridge, skip_4, 512, 'decoder_1', dropout_rate)
    x = enhanced_decoder_block_with_msca_lbfr(x, skip_3, 256, 'decoder_2', dropout_rate)
    x = enhanced_decoder_block_with_msca_lbfr(x, skip_2, 128, 'decoder_3', dropout_rate * 0.7)
    x = enhanced_decoder_block_with_msca_lbfr(x, skip_1, 64, 'decoder_4', dropout_rate * 0.5)
    ...
    print(f"  Dual Attention Modules: MSCA + LBFR per decoder block")
    print(f"  MSCA Modules: 4 (multi-scale context with dilation rates [1,2,3,5])")
    print(f"  LBFR Modules: 4 (channel + spatial attention + recalibration)")
    print(f"  Enhancement Strategy: Sequential MSCA → LBFR per block")
    print(f"  Total Attention Blocks: 8 (4 MSCA + 4 LBFR)")
...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_name, 
    ...
    """Train model with MSCA + LBFR dual enhancement"""
    print(f"\n{'='*80}")
    print(f"TRAINING: {model_name}")
    print(f"{'='*80}")
    print(f"  Loss: Focal + EIoU")
    print(f"  Architecture: Deeper Decoder + MC Dropout + MSCA + LBFR")
    print(f"  MSCA: Multi-scale Context Aggregator (dilated convs [1,2,3,5])")
    print(f"  LBFR: Lightweight Bidirectional Feature Recalibrator")
    print(f"  Enhancement: Sequential MSCA → LBFR per decoder block")
    
    K.clear_session()
    
    # Build model
    model = build_unet_with_msca_lbfr(dropout_rate=0.3)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*100}")
    print(f"{'MOBILENET-UNET WITH MSCA + LBFR (DUAL ATTENTION MODULES)':^100s}")
    print(f"{'='*100}")
    ...
    model, history, params = train_model(
        ...
        "Deeper_Focal_MC_MSCA_LBFR",
        augment=train_aug, save_dir=save_dir
    )
    
    # Evaluate
    results = evaluate_model(model, X_test, y_test, "Deeper+Focal+MC+MSCA+LBFR", run_mc_dropout=True)
    
    print(f"\n{'='*100}")
    print(f"{'✓ TRAINING AND EVALUATION COMPLETED!':^100s}")
    print(f"{'='*100}")
    print(f"\nModel: Deeper + Focal + EIoU + MC Dropout + MSCA + LBFR")
    print(f"Dual Enhancement: MSCA (Multi-scale Context) + LBFR (Bidirectional Recalibration)")
    print(f"MSCA: Multi-scale dilated convolutions with rates [1, 2, 3, 5]")
    print(f"LBFR: Channel + Spatial attention + Recalibration")
    print(f"Total Attention Blocks: 8 (4 MSCA + 4 LBFR)")
