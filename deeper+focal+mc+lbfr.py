"""
MOBILENET-UNET WITH LBFR (LIGHTWEIGHT BIDIRECTIONAL FEATURE RECALIBRATOR)
==========================================================================
Enhanced with LBFR from ODFormer for bidirectional feature recalibration

LBFR Module:
- Channel attention (avg + max pooling)
- Spatial attention (2-channel concatenation)
- Recalibration layer with residual connection
- From ODFormer architecture (Swin + MSCA + LBFR)

Configuration:
- Base: Deeper Decoder + Focal-EIoU + MC Dropout
- Enhancement: LBFR on decoder outputs
- LBFR Placement: After each decoder block (4 total)
"""
...
# ============================================================================
# MODEL ARCHITECTURE WITH LBFR
# ============================================================================

def enhanced_decoder_block_with_lbfr(x, skip_connection, filters, block_name, dropout_rate=0.3):
    """Enhanced decoder block with LBFR for bidirectional feature recalibration"""
...
def build_unet_with_lbfr(input_shape=(256, 256, 3), num_classes=3, dropout_rate=0.3):
    """Build MobileNetV2-UNet with LBFR modules"""
    ...
    # Decoder with LBFR modules
    x = enhanced_decoder_block_with_lbfr(bridge, skip_4, 512, 'decoder_1', dropout_rate)
    x = enhanced_decoder_block_with_lbfr(x, skip_3, 256, 'decoder_2', dropout_rate)
    x = enhanced_decoder_block_with_lbfr(x, skip_2, 128, 'decoder_3', dropout_rate * 0.7)
    x = enhanced_decoder_block_with_lbfr(x, skip_1, 64, 'decoder_4', dropout_rate * 0.5)
    ...
    print(f"  LBFR Modules: 4 (one per decoder block)")
    print(f"  LBFR Components: Channel Attention + Spatial Attention + Recalibration")
    print(f"  Enhancement: Bidirectional Feature Recalibration from ODFormer")
...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_name, 
    ...
    """Train model with LBFR enhancement"""
    print(f"\n{'='*80}")
    print(f"TRAINING: {model_name}")
    print(f"{'='*80}")
    print(f"  Loss: Focal + EIoU")
    print(f"  Architecture: Deeper Decoder + MC Dropout + LBFR")
    print(f"  LBFR: Lightweight Bidirectional Feature Recalibrator from ODFormer")
    
    K.clear_session()
    
    # Build model
    model = build_unet_with_lbfr(dropout_rate=0.3)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*100}")
    print(f"{'MOBILENET-UNET WITH LBFR (LIGHTWEIGHT BIDIRECTIONAL FEATURE RECALIBRATOR)':^100s}")
    print(f"{'='*100}")
    ...
    model, history, params = train_model(
        ...
        "Deeper_Focal_MC_LBFR",
        augment=train_aug, save_dir=save_dir
    )
    
    # Evaluate
    results = evaluate_model(model, X_test, y_test, "Deeper+Focal+MC+LBFR", run_mc_dropout=True)
    
    print(f"\n{'='*100}")
    print(f"{'✓ TRAINING AND EVALUATION COMPLETED!':^100s}")
    print(f"{'='*100}")
    print(f"\nModel: Deeper + Focal + EIoU + MC Dropout + LBFR")
    print(f"Enhancement: Lightweight Bidirectional Feature Recalibrator from ODFormer")
    print(f"LBFR Components: Channel Attention + Spatial Attention + Recalibration")
