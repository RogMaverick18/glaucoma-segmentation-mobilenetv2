"""
COMBINED ADVANCED MOBILENET-UNET WITH SKIP-ONLY CBAM ATTENTION
================================================================
This combines:
1. Deeper decoder architecture with enhanced blocks
2. Focal-EIoU loss
3. MC Dropout for uncertainty estimation
4. CBAM (Convolutional Block Attention Module) - SKIP-ONLY VERSION
   - Applied ONLY to: Skip connections (4 locations)
   - REMOVED from: bridge, decoder blocks, final layer
   - This applies attention to encoder features before they merge with decoder

STRATEGY: Place CBAM only on skip connections to refine encoder features
before concatenation with decoder features, allowing better feature fusion.
"""
...
# ============================================================================
# ENHANCED DECODER BLOCK (WITHOUT CBAM - for skip-only version)
# ============================================================================
...
def enhanced_decoder_block_no_cbam(x, skip_connection, filters, block_name, dropout_rate=0.3):
    ...
    # NO CBAM in decoder blocks for skip-only version
    
    return x

# ============================================================================
# MODEL: SKIP-ONLY CBAM VERSION (4 blocks on skip connections)
# ============================================================================

def build_combined_unet_skip_only_cbam(input_shape=(256, 256, 3), num_classes=3, 
                                       use_deeper=False, dropout_rate=0.3):
    """
    Combined MobileNet-UNet with SKIP-ONLY CBAM
    
    CBAM Placement (SKIP-ONLY VERSION):
    - Skip_1 (128x128): YES ✅
    - Skip_2 (64x64): YES ✅
    - Skip_3 (32x32): YES ✅
    - Skip_4 (16x16): YES ✅
    ...
    - Final layer: NO CBAM ❌
    Total: 4 CBAM blocks (on skip connections only)
    
    Rationale: Apply attention to encoder features before they merge with decoder,
    allowing the network to select important features for skip connections.
    """
    
    print(f"\nBuilding Combined UNet with SKIP-ONLY CBAM...")
    print(f"  Deeper Architecture: {use_deeper}")
    print(f"  MC Dropout Rate: {dropout_rate}")
    print(f"  CBAM Attention: SKIP-ONLY (4 blocks on skip connections)")
    
    inputs = layers.Input(input_shape, dtype='float32')
    backbone = MobileNetV2(input_tensor=inputs, weights='imagenet', include_top=False)
    
    # Get skip connections and apply CBAM
    ...
    print("    skip_1 (128x128): WITH CBAM ✓")
    print("    skip_2 (64x64): WITH CBAM ✓")
    print("    skip_3 (32x32): WITH CBAM ✓")
    print("    skip_4 (16x16): WITH CBAM ✓")
    
    # Apply CBAM to ALL skip connections
    skip_1 = cbam_block(skip_1_raw, ratio=8, name="skip1")
    skip_2 = cbam_block(skip_2_raw, ratio=8, name="skip2")
    skip_3 = cbam_block(skip_3_raw, ratio=8, name="skip3")
    skip_4 = cbam_block(skip_4_raw, ratio=8, name="skip4")
    
    # Deeper bridge (if enabled) - NO CBAM on bridge
    if use_deeper:
    ...
    if use_deeper:
        print("  Building deeper decoder WITHOUT CBAM (attention only on skips)...")
        # All decoder blocks WITHOUT CBAM
    ...
    print(f"\n  CBAM Configuration (SKIP-ONLY):")
    print(f"    Total CBAM Components: {cbam_count}")
    print(f"    CBAM Locations: Skip connections ONLY (4 blocks)")
    print(f"    Removed CBAM from: Bridge, Decoder blocks, Final layer")
...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_name, 
    ...
    """Train model with skip-only CBAM"""
    print(f"\n{'='*80}")
    ...
    print(f"  CBAM Attention: ✓ SKIP-ONLY (4 skip connections)")
    
    K.clear_session()
    
    model = build_combined_unet_skip_only_cbam(use_deeper=use_deeper, dropout_rate=dropout_rate)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*100}")
    print(f"{'EXPERIMENT: DEEPER + FOCAL-EIOU + MC DROPOUT + SKIP-ONLY CBAM':^100s}")
    print(f"{'='*100}")
    ...
    print(f"\n{'='*80}")
    print("EXPERIMENT: FULL COMBINATION + SKIP-ONLY CBAM (4 blocks on skips)")
    print(f"{'='*80}")
    
    model, history, params = train_model(
        X_train, y_train, X_val, y_val, X_test, y_test,
        "FULL_with_SKIP_ONLY_CBAM",
        use_focal_eiou=True, use_deeper=True, dropout_rate=0.3,
        augment=train_aug, save_dir=save_dir
    )
    
    all_results['FULL + SKIP-ONLY CBAM'] = evaluate_model(model, X_test, y_test, "Full with Skip-Only CBAM", run_mc_dropout=True, save_dir=save_dir)
    all_params['FULL + SKIP-ONLY CBAM'] = params
