"""
V8: V5 BASE + CLAHE + MILD NOISE + POST-PROCESSING + TTA
====================================================================
V5's proven training setup with targeted improvements:

Phase 1 — Targeted Augmentation (V5 base + 2 safe additions):
  - V5's exact augmentation pipeline (proven to give Disc Dice=0.9349)
  - + CLAHE (fundus contrast normalization, no geometry change)
  - + Mild GaussNoise (small variance, doesn't deform boundaries)
  - ROI padding: 1.2× (same as V5)
  - NO offline augmentation (~600 training images, ~1hr training)

Phase 2 — Post-Processing (on-the-go during evaluation):
  - Largest connected component selection
  - Morphological closing (hole filling)
  - Contour-based hole filling
  - Median filter for smoother boundaries
  - Anatomical constraint: cup ⊂ disc

Phase 3 — Test-Time Augmentation:
  - 4-pass flip ensemble (orig, hflip, vflip, both)
  - Average softmax → argmax → post-process

Training config UNCHANGED from V5:
  - Adam lr=2e-4, ReduceLROnPlateau, 80 epochs, patience=15
  - Focal loss alpha=[0.25, 1.0, 1.0] + Enhanced IoU
  - No label smoothing, no cosine annealing
...
def visualize_roi_extraction(images, masks, num_samples=4):
    ...
    for i in range(min(num_samples, len(images))):
        ...
        y1, x1, y2, x2 = coords
        axes[i, 0].imshow(images[i]); axes[i, 0].set_title('Original Image', fontsize=11, fontweight='bold')
        rect = plt.Rectangle((x1, y1), x2-x1, y2-y1, linewidth=2, edgecolor='lime', facecolor='none')
        axes[i, 0].add_patch(rect); axes[i, 0].axis('off')
        axes[i, 1].imshow(masks[i], cmap='jet', vmin=0, vmax=2); axes[i, 1].set_title('Original Mask', fontsize=11, fontweight='bold')
        rect2 = plt.Rectangle((x1, y1), x2-x1, y2-y1, linewidth=2, edgecolor='lime', facecolor='none')
        axes[i, 1].add_patch(rect2); axes[i, 1].axis('off')
        axes[i, 2].imshow(roi_img); axes[i, 2].set_title(f'ROI Image ({roi_img.shape[0]}×{roi_img.shape[1]})', fontsize=11, fontweight='bold'); axes[i, 2].axis('off')
        axes[i, 3].imshow(roi_msk, cmap='jet', vmin=0, vmax=2); axes[i, 3].set_title(f'ROI Mask ({roi_msk.shape[0]}×{roi_msk.shape[1]})', fontsize=11, fontweight='bold'); axes[i, 3].axis('off')
    plt.suptitle('ROI Extraction Verification', fontsize=14, fontweight='bold', y=1.01)
...
def setup_augmentations():
    ...
    - CLAHE: fundus contrast normalization (intensity only, no geometry change)
    - GaussNoise (mild): small variance, doesn't deform boundaries
    
    NOT included (proven to hurt disc Dice in V8 experiments):
    - RandomRotate90 (unrealistic for fundus)
    - GridDistortion (too aggressive)
    - OpticalDistortion (distorts boundaries)
    - CoarseDropout (masks boundary pixels)
    """
    return A.Compose([
...
def data_generator(images, masks, batch_size, augment=None):
    """Data generator — same as V5 (no label smoothing)."""
...
def postprocess_prediction(pred_mask):
    ...
    Applied on-the-go during evaluation, before computing Dice/IoU.
    
    1. Largest connected component (remove spurious regions)
    2. Morphological closing (fill small boundary gaps)
    3. Contour-based hole filling
    4. Median filter (smoother boundaries)
    5. Anatomical constraint: cup ⊂ disc
    """
    cleaned = pred_mask.copy()
    
    for class_id in [1, 2]:
        if class_id == 1:
            # For disc: consider disc AND cup combined
...
# ============================================================================
# LOSS FUNCTIONS (identical to V5)
# ============================================================================
...
# ============================================================================
# TRAINING (same as V5: Adam 2e-4, ReduceLROnPlateau, 80 epochs, patience=15)
# ============================================================================
...
def train_model(X_train_roi, y_train_roi, X_val_roi, y_val_roi,
    ...
    print(f"{'='*80}")
    print(f"  Config: V5 base (Adam 2e-4, ReduceLROnPlateau, 80 epochs, patience=15)")
    print(f"  Loss: Focal [0.25, 1.0, 1.0] + Enhanced IoU")
    print(f"  Aug: V5 base + CLAHE + mild GaussNoise")
    ...
    reduce_lr = ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=4, verbose=1, mode='min', min_lr=1e-7)
    early_stop = EarlyStopping(monitor="val_loss", patience=15, verbose=1, restore_best_weights=True, mode='min')
...
def evaluate_model(model, X_test_roi, y_test_roi, y_test_full, roi_coords_test,
    ...
    print("Raw metrics:")
    for cn in class_names: print(f"  {cn}: Dice={raw[cn]['dice']:.4f}, IoU={raw[cn]['iou']:.4f}")
    
    # Stage 2: Post-processed
    print(f"\n{'─'*80}\nSTAGE 2: POST-PROCESSED\n{'─'*80}")
    y_pred_pp = postprocess_batch(y_pred_raw)
    pp = compute_dice_iou_per_class(y_test_roi, y_pred_pp)
    print("Post-processed metrics:")
    for cn in class_names:
        d = pp[cn]['dice']-raw[cn]['dice']; print(f"  {cn}: Dice={pp[cn]['dice']:.4f} ({d:+.4f})")
    ...
    print("TTA+PP metrics:")
    for cn in class_names:
        d = tta[cn]['dice']-raw[cn]['dice']; print(f"  {cn}: Dice={tta[cn]['dice']:.4f} ({d:+.4f} vs raw)")
    ...
    precision = precision_score(y_test_roi.flatten(), y_best.flatten(), average=None, zero_division=0)
    recall = recall_score(y_test_roi.flatten(), y_best.flatten(), average=None, zero_division=0)
    f1 = f1_score(y_test_roi.flatten(), y_best.flatten(), average=None, zero_division=0)
    print(f"\nPer-class (TTA+PP):")
    for i, cn in enumerate(class_names):
        print(f"  {cn}: Precision={precision[i]:.4f}, Recall={recall[i]:.4f}, F1={f1[i]:.4f}")
    print(f"\n  {class_names[0]}: Dice={tta['Background']['dice']:.4f}, IoU={tta['Background']['iou']:.4f}")
    print(f"  {class_names[1]}: Dice={tta['Disc']['dice']:.4f}, IoU={tta['Disc']['iou']:.4f}")
    print(f"  {class_names[2]}: Dice={tta['Cup']['dice']:.4f}, IoU={tta['Cup']['iou']:.4f}")
...
if __name__ == "__main__":
    ...
    MODEL_NAME = "V8_V5base_CLAHE_PP_TTA"
    PADDING_FACTOR = 1.2
    
    print(f"\n{'='*100}")
    print(f"{'V8: V5 BASE + CLAHE + MILD NOISE + PP + TTA':^100s}")
    print(f"{'='*100}")
    print(f"\nPhase 1 — Targeted Augmentation:")
    print(f"  - V5's exact augmentation pipeline (proven Disc Dice=0.9349)")
    print(f"  - + CLAHE (contrast normalization)")
    print(f"  - + Mild GaussNoise (var 5-25)")
    print(f"  - ROI padding: {PADDING_FACTOR}× (same as V5)")
    print(f"  - NO offline augmentation (~600 images, ~1hr training)")
    print(f"\nPhase 2 — Post-Processing:")
    print(f"  - Largest CC → Morphological closing → Hole filling → Median filter → Cup⊂Disc")
    print(f"\nPhase 3 — TTA: 4-pass flip ensemble")
    print(f"\nTraining: V5 config (Adam 2e-4, ReduceLROnPlateau, 80 epochs, patience=15)")
    print(f"Loss: Focal [0.25, 1.0, 1.0] + Enhanced IoU")
    ...
    print(f"{'='*100}")
    print(f"  Model: V8 — V5 base + CLAHE + mild noise + PP + TTA")
