"""
COMBINED ADVANCED MOBILENET-UNET WITH ENCODER-ONLY CBAM ATTENTION
===================================================================
This combines:
1. Deeper decoder architecture with enhanced blocks
2. Focal-EIoU loss
3. MC Dropout for uncertainty estimation
4. CBAM (Convolutional Block Attention Module) - ENCODER-ONLY VERSION
   - Applied ONLY to: Encoder outputs (4 locations + bridge)
   - REMOVED from: decoder blocks, final layer
   - Each encoder output gets attention ONCE, then flows to both skip and next encoder

STRATEGY: Place CBAM after each encoder block to refine encoder features.
...
# ============================================================================
# ENHANCED DECODER BLOCK (WITHOUT CBAM - for encoder-only version)
# ============================================================================
...
def enhanced_decoder_block_no_cbam(x, skip_connection, filters, block_name, dropout_rate=0.3):
    ...
    # NO CBAM in decoder blocks for encoder-only version
    
    return x

# ============================================================================
# MODEL: ENCODER-ONLY CBAM VERSION (5 blocks: 4 encoder outputs + bridge)
# ============================================================================

def build_combined_unet_encoder_only_cbam(input_shape=(256, 256, 3), num_classes=3, 
                                          use_deeper=False, dropout_rate=0.3):
    """
    Combined MobileNet-UNet with ENCODER-ONLY CBAM
    
    CBAM Placement (ENCODER-ONLY VERSION):
    ...
    - Decoder blocks: NO CBAM ❌
    - Final layer: NO CBAM ❌
    Total: 5 CBAM blocks (on encoder outputs only)
    ...
    print("  Applying CBAM to encoder outputs...")
    print("    encoder_1 (128x128): WITH CBAM ✓ → flows to skip_1")
    print("    encoder_2 (64x64): WITH CBAM ✓ → flows to skip_2")
    print("    encoder_3 (32x32): WITH CBAM ✓ → flows to skip_3")
    print("    encoder_4 (16x16): WITH CBAM ✓ → flows to skip_4")
    print("    bridge (8x8): WITH CBAM ✓")
    
    # Apply CBAM to ALL encoder outputs (these become the skip connections)
    skip_1 = cbam_block(enc_1_raw, ratio=8, name="enc1")
    skip_2 = cbam_block(enc_2_raw, ratio=8, name="enc2")
    skip_3 = cbam_block(enc_3_raw, ratio=8, name="enc3")
    skip_4 = cbam_block(enc_4_raw, ratio=8, name="enc4")
    bridge = cbam_block(bridge_raw, ratio=8, name="bridge")
    
    # Deeper bridge (if enabled) - applied AFTER CBAM
    if use_deeper:
    ...
    if use_deeper:
        print("  Building deeper decoder WITHOUT CBAM (attention only on encoder outputs)...")
        # All decoder blocks WITHOUT CBAM (use attention-refined skip connections)
        ...
        x = enhanced_decoder_block_no_cbam(x, skip_2, 128, 'decoder_3', dropout_rate * 0.7)
        x = enhanced_decoder_block_no_cbam(x, skip_1, 64, 'decoder_4', dropout_rate * 0.5)
    else:
        print("  Building baseline decoder WITHOUT CBAM...")
        # Baseline decoder without CBAM
    ...
    print(f"\n  CBAM Configuration (ENCODER-ONLY):")
    print(f"    Total CBAM Components: {cbam_count}")
    print(f"    CBAM Locations: Encoder outputs ONLY (5 blocks)")
    print(f"    Removed CBAM from: Decoder blocks, Final layer")
    print(f"    Note: Each encoder output gets CBAM once, then flows to skip connection")
...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_name, 
    ...
    """Train model with encoder-only CBAM"""
    print(f"\n{'='*80}")
    ...
    print(f"  CBAM Attention: ✓ ENCODER-ONLY (5 encoder outputs)")
    
    K.clear_session()
    
    model = build_combined_unet_encoder_only_cbam(use_deeper=use_deeper, dropout_rate=dropout_rate)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*100}")
    print(f"{'EXPERIMENT: DEEPER + FOCAL-EIOU + MC DROPOUT + ENCODER-ONLY CBAM':^100s}")
    print(f"{'='*100}")
    ...
    print(f"\n{'='*80}")
    print("EXPERIMENT: FULL COMBINATION + ENCODER-ONLY CBAM (5 blocks on encoder outputs)")
    print(f"{'='*80}")
    
    model, history, params = train_model(
        X_train, y_train, X_val, y_val, X_test, y_test,
        "FULL_with_ENCODER_ONLY_CBAM",
        use_focal_eiou=True, use_deeper=True, dropout_rate=0.3,
        augment=train_aug, save_dir=save_dir
    )
    
    all_results['FULL + ENCODER-ONLY CBAM'] = evaluate_model(model, X_test, y_test, "Full with Encoder-Only CBAM", run_mc_dropout=True, save_dir=save_dir)
    all_params['FULL + ENCODER-ONLY CBAM'] = params
