"""
MOBILENET-UNET WITH MSCA (MULTI-SCALE CONTEXT AGGREGATOR)
===========================================================
Enhanced with MSCA from ODFormer for multi-scale feature aggregation

MSCA Module:
- Multi-scale dilated convolutions (rates: 1, 2, 3, 5)
- Captures features at different receptive fields
- Lightweight feature fusion with residual connection
- From ODFormer architecture (Swin + MSCA + LBFR)

Configuration:
- Base: Deeper Decoder + Focal-EIoU + MC Dropout
- Enhancement: MSCA on decoder outputs
- MSCA Placement: After each decoder block (4 total)
"""
...
def msca_block(x, channels, dilation_rates=[1, 2, 3, 5], block_name='msca'):
    ...
    Features:
    - Multi-scale dilated convolutions for different receptive fields
...
# ============================================================================
# MODEL ARCHITECTURE WITH MSCA
# ============================================================================

def enhanced_decoder_block_with_msca(x, skip_connection, filters, block_name, dropout_rate=0.3):
    """Enhanced decoder block with MSCA for multi-scale context aggregation"""
...
def build_unet_with_msca(input_shape=(256, 256, 3), num_classes=3, dropout_rate=0.3):
    """Build MobileNetV2-UNet with MSCA modules"""
    ...
    # Decoder with MSCA modules
    x = enhanced_decoder_block_with_msca(bridge, skip_4, 512, 'decoder_1', dropout_rate)
    x = enhanced_decoder_block_with_msca(x, skip_3, 256, 'decoder_2', dropout_rate)
    x = enhanced_decoder_block_with_msca(x, skip_2, 128, 'decoder_3', dropout_rate * 0.7)
    x = enhanced_decoder_block_with_msca(x, skip_1, 64, 'decoder_4', dropout_rate * 0.5)
    ...
    print(f"  MSCA Modules: 4 (one per decoder block)")
    print(f"  MSCA Dilation Rates: [1, 2, 3, 5]")
    print(f"  Enhancement: Multi-scale Context Aggregation from ODFormer")
...
def train_model(X_train, y_train, X_val, y_val, X_test, y_test, model_name, 
    ...
    """Train model with MSCA enhancement"""
    print(f"\n{'='*80}")
    print(f"TRAINING: {model_name}")
    print(f"{'='*80}")
    print(f"  Loss: Focal + EIoU")
    print(f"  Architecture: Deeper Decoder + MC Dropout + MSCA")
    print(f"  MSCA: Multi-scale Context Aggregator from ODFormer")
    
    K.clear_session()
    
    # Build model
    model = build_unet_with_msca(dropout_rate=0.3)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*100}")
    print(f"{'MOBILENET-UNET WITH MSCA (MULTI-SCALE CONTEXT AGGREGATOR)':^100s}")
    print(f"{'='*100}")
    ...
    model, history, params = train_model(
        ...
        "Deeper_Focal_MC_MSCA",
        augment=train_aug, save_dir=save_dir
    )
    
    # Evaluate
    results = evaluate_model(model, X_test, y_test, "Deeper+Focal+MC+MSCA", run_mc_dropout=True)
    
    print(f"\n{'='*100}")
    print(f"{'✓ TRAINING AND EVALUATION COMPLETED!':^100s}")
    print(f"{'='*100}")
    print(f"\nModel: Deeper + Focal + EIoU + MC Dropout + MSCA")
    print(f"Enhancement: Multi-scale Context Aggregator from ODFormer")
    print(f"MSCA Dilation Rates: [1, 2, 3, 5]")
